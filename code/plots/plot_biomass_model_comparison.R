...
chains <- read_csv('data/model_fits/hierarchical_model_posterior_distributions_for_plot_loggpp_logbm_fixedK600_gamma.csv')
chains <- data.frame(parameter = c(rep('gamma_fila',2),
                                   rep('gamma_epil',2)),
                     chains = rep(0,4),
                     biomass_vars = rep(NA_character_,4),
...
ggpubr::ggarrange(log_K, lin_K,nrow = 2, common.legend = TRUE)
