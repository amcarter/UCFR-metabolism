...
NPP <- met %>%
    group_by(site, year) %>%
    summarize(across(any_of(c('GPP', 'ER', 'ARf', 'NPP', 'NPP_globalARf',
                              'AR', 'HR')),
                     mean, na.rm = T))
...
npp %>%
    pivot_longer(cols = c('GPP', 'ER', 'ARf', 'NPP_C', 'NPP_C_globalARf',
...
png('figures/algal_assemblage_relationships.png',
    ...
    npp %>%
        select(site, year, fila_bloom, frac_fila_gm, A_GPP_C = GPP_C, B_ER_C = ER_C,
        ...
        # ggplot(aes(frac_fila_gm, value, pch = factor(fila_bloom))) +
        geom_point(size = 1.6) +
        facet_wrap(response~., scales = 'free_y',
                   strip.position = 'left',
                   nrow = 3,
                   labeller = as_labeller(c(A_GPP_C = 'GPP (gC/m2/d)', B_ER_C = 'ER (gC/m2/d)',
                                            C_ARf = 'Autotrophic Respiration Fraction',
                                            D_NPP_C = 'NPP (gC/m2/d)',
                                            E_biomass_C = 'Biomass (gC/m2)',
...
p2 <- ggplot(npp, aes(fila_gm/2/(biomass_C)*100, days_bio))+
    ...
    theme(legend.title = element_blank(),
          ...
          panel.border = element_rect(fill = NA))
p2B <- ggplot(npp, aes(fila_gm/2/(biomass_C)*100, NPP_C))+
    xlab('Filamentous fraction of \ntotal biomass (%)')+
    ylab('Mean daily production (gC/m2/d)')+
...
ggpubr::ggarrange(p2B, p2, common.legend = TRUE)
dev.off()
