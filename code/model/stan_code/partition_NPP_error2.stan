...
transformed parameters {
    ...
    // prior distributions:
    lprior += normal_lpdf(mu_f | 0, 0.3);
    lprior += normal_lpdf(mu_e | 0, 0.1);
    // lprior += normal_lpdf(K_I | 5, 1);
    lprior += student_t_lpdf(sigma | 3, 0, 2.5)
        - 1 * student_t_lccdf(0 | 3, 0, 2.5);
    lprior += normal_lpdf(meanme[1] | 50, 10);
    lprior += normal_lpdf(meanme[2] | 30, 10);
    lprior += student_t_lpdf(sdme | 3, 0, 2.5)
...
model {
    ...
    for(n in 1:N){
        ...
        mu[n] += (mu_f * B_f[n] + mu_e * B_e[n]);// .* (light[n]./(K_I + light[n]));
    }
...
//        NPP_post[i] = normal_rng((mu_f*B_f[i] * (1/(1 + K_Bf * B_f[i])) + mu_e*B_e[i]) * (light[i]/(K_I + light[i])), sigma);
        NPP_post[i] = normal_rng((mu_f*B_f[i] + mu_e*B_e[i]), sigma);// * (light[i]/(K_I + light[i])), sigma);
    }
}
