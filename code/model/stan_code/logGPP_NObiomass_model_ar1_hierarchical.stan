...
parameters {
    ...
    vector[3] gamma;          // population level coefficients
...
model {
    ...
    for(n in 2:N){
        if(new_ts[n] == 1){
            ...
            mu[n] ~ normal(beta[ss[n]] + gamma[2]*K600[n] + gamma[3]* light[n] + phi*mu[n-1], sigma);
        }
    }
...
generated quantities {
    ...
    for(n in 2:N){
        if(new_ts[n] == 1){
            ...
            mu_tilde[n] = normal_rng(beta[ss[n]] + gamma[2]*K600[n] + gamma[3]*light[n] + phi*mu_tilde[n-1], sigma);
        }
