...
parameters {
    vector[4] gamma;          // population level coefficients
...
model {
    ...
    for(n in 2:N){
        mu[n] ~ normal(beta[ss[n]] + gamma[2]* light[n] + gamma[3]*biomass[n]+ gamma[4] * biomass[n]* light[n], sigma);
    }
...
generated quantities {
    ...
    for(n in 2:N){
        mu_tilde[n] = normal_rng(beta[ss[n]] + gamma[2]*light[n] + gamma[3]*biomass[n] + gamma[4]*biomass[n]* light[n], sigma);
