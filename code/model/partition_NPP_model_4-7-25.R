...
coefs <- coef(modmix_brm)
coefs_epil <- data.frame(coefs$siteyear[,,1])
colnames(coefs_epil) <- paste(colnames(coefs_epil), "epil", sep = "_")
coefs_epil$siteyear <- rownames(coefs_epil)
coefs_fila <- data.frame(coefs$siteyear[,,2])
colnames(coefs_fila) <- paste(colnames(coefs_fila), "fila", sep = "_")
coefs_fila$siteyear <- rownames(coefs_fila)

bm_met %>% left_join(coefs, by = "siteyear") %>%
    mutate(fila_prod_gCmd = Estimate_fila * filacl,
           fila_prod_gCmd.low = Q2.5_fila * filacl,
           fila_prod_gCmd.high = Q97.5_fila * filacl,
           epil_prod_gCmd = Estimate_epil * epilcl,
           epil_prod_gCmd.low = Q2.5_epil * epilcl,
           epil_prod_gCmd.high = Q97.5_epil * epilcl
           )
