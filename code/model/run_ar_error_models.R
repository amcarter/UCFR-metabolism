...
for(i in 1:length(which(new_ts == 1))){
    ...
    y[start:end] = arima.sim(model = list(ar = phi), n = end-start+1) +
...
ma1_mod <- stan_model('code/model/stan_code/MA1.stan',
                      model_name = 'ma_mod')
ar_mod <- stan_model('code/model/stan_code/AR1.stan',
                       model_name = 'ar_mod')
datlist <- list(
    N = N, K = K, S = S,
...
print(ar_fit, pars = c('sigma', 'phi', 'gamma', 'beta', 'tau'))
sigma
phi
beta
beta0
...
pairs(ar_fit, pars = c('sigma', 'phi', 'gamma', 'tau'))

shinystan::launch_shinystan(ar_fit)
...
X_empty <- matrix(nrow = nrow(X), ncol = 0)
fit <- fit_biomass_model(ar_mod, dd, X = X_empty,
                         log_GPP = TRUE, iter = 4000)
...
for(i in 1:length(model_combinations)){
    ...
    fit <- fit_biomass_model(ar_mod, dd,
                             X = as.matrix(X),
                             log_GPP = TRUE,
                             iter = 4000)
    preds <- get_model_preds(fit, dd)
    ...
    ests <- extract_model_params(fit, pars = c('phi','gamma', 'beta', 'sigma', 'tau'),
    ...
    comp <- calculate_model_metrics(fit, X, dd, 'ar_err_mod')
...
write_csv(mod_ests, 'data/model_fits/ar_err_log_model_parameter_ests_ss.csv')
write_csv(chains, 'data/model_fits/ar_err_log_model_chains_ss.csv')
write_csv(mod_comp, 'data/model_fits/ar_err_log_model_metrics_ss_cond.csv')
...
for(i in 1:length(model_combinations)){
    ...
    for(s in sites){
        ...
        # X <- master_X[,model_combinations[[i]]]
        fit <- fit_biomass_model(ar_mod, dd[-holdout_rows,],
...
write_csv(mod_holdout_preds, 'data/model_fits/ar_err_log_model_holdout_predictions.csv')
write_csv(mod_post_preds, 'data/model_fits/ar_err_log_model_holdout_post_predictions.csv')
write_csv(mod_holdout_RMSE, 'data/model_fits/ar_err_log_model_holdout_RMSEs.csv')

mod_holdout_preds <- read_csv('data/model_fits/ar_err_log_model_holdout_predictions.csv')
mod_post_preds <- read_csv('data/model_fits/ar_err_log_model_holdout_post_predictions.csv')
mod_holdout_preds_ma <- read_csv('data/model_fits/ma1_log_model_holdout_predictions.csv')
...
mod_holdout_preds  %>% tibble() %>%
    ...
    mutate(se = (exp(log_GPP) - exp(estim))^2) %>%
